...
| **Cloud models, if you want them** | Off unless you add a provider: OpenRouter, OpenAI, Anthropic, DeepSeek, or anything with the OpenAI API, including LM Studio and Ollama. Pick its models from the list it publishes; they join the Speech and Cleanup choices. A slot given a cloud model loads no local model into graphics memory; cleanup falls back to the local one, loaded then, if the cloud fails. API keys live in the system credential store, never in the config file, and go only to the provider's address over https. |
